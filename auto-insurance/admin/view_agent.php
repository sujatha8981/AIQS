...
<head>
    ...
    <!--title-->
        <title>View Agents | MCVT Insurance Services</title>
    <!--icons-->
...
<main class="grey lighten-3" style="min-height:80vh">
    ...
    <div class="row"><!--first row-->
        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6 col-xl-6"><!--first col of 1st row-->
            <h4 class="page-title">View Agents</h4>
    ...
    <div class="card mx-5"><!--card-->
        <div class="card-body">
                ...
                <thead>
                    <tr>
                        ...
                        <tbody>
                               	<?php
									include "connect.php";
                                    $view_agents = "SELECT * FROM agents";
                                    $result = mysqli_query($conn, $view_agents);
                                    while($row=$result->fetch_assoc()){
								?>
                                <tr>
                                        <td class="wo-button"><a class="view-agent font-weight-bold material-tooltip-main" data-toggle="tooltip" data-placement="right" title="View Agent" href="view_individual_agent.php?id=<?=$row['id']?>"><?php echo $row["agent_fname"] . " " . $row["agent_mname"] . " " . $row["agent_lname"]; ?></a></td><!--place a tooltip-->
                                        <td class="wo-button"><?=$row["agent_mobile_no"]?></td>
                                        <td class="wo-button"><?=$row["agent_email"]?></td>
                                        <td class="with-button">
                                            <button type="button" class="btn btn-sm btn-info"><a class="text-white" href="update_agent.php?id=<?=$row['id']?>"><span class="fas fa-edit"></span> Update</a></button>
                                            <button onClick="change(<?=$row['id']?>)" type="button" data-toggle="modal" data-target="#modal-delete-agent" class="btn btn-danger btn-sm"><span class="fas fa-trash-alt"></span> Delete</button>
                                        </td>
                                </tr>
                                <?php
									}
            ...
            <!--modal for delete-->
            <div class="modal fade" id="modal-delete-agent" tabindex="-1" role="dialog" aria-labelledby="modal-label" aria-hidden="true">
                <div class="modal-dialog modal-notify modal-danger" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title w-100" id="modal-label"><span class="heading-lead text-white">Remove Agent</span></h5>
                            ...
                            <div class="modal-body">
                                ...
                                <div class="modal-footer justify-content-center">
									<script>
										function change(id){
											history.pushState({}, "", "view_agent.php?id=" + id);
											
											document.getElementById("deletebutton").innerHTML = '<form action="delete.php?id='+id+'" method="post"><button type="button" class="btn btn-info" data-dismiss="modal">Nope</button><input type="submit" class="btn btn-danger" value="Yes, Remove!" name="delete_agent">';
										}
									</script>
